...
// WinRT initialization flag
bool winrtInitialized = false;

// Initialize WinRT runtime
bool initializeWinRT() {
    if (winrtInitialized) {
        return true;
    }
    ...
    if (SUCCEEDED(hr)) {
        ...
        writeDebugLog("WinRT runtime initialized successfully");
        return true;
    } else {
        writeDebugLog("Failed to initialize WinRT runtime. Error: " + std::to_string(hr));
        return false;
    }
}

// Cleanup WinRT runtime
void cleanupWinRT() {
    if (winrtInitialized) {
        RoUninitialize();
        winrtInitialized = false;
        writeDebugLog("WinRT runtime cleaned up");
    }
}
...
void showToastNotification(const std::string& title, const std::string& message, const std::string& type) {
    if (!winrtInitialized) {
        writeDebugLog("WinRT not initialized, skipping toast notification");
        return;
    }
    
    try {
        // Create XML template for toast notification
        std::wstring xmlTemplate = L"<toast>"
            L"<visual>"
            L"<binding template='ToastGeneric'>"
            L"<text id='1'>" + std::wstring(title.begin(), title.end()) + L"</text>"
            L"<text id='2'>" + std::wstring(message.begin(), message.end()) + L"</text>"
            L"</binding>"
            L"</visual>"
            L"</toast>";
        ...
        HRESULT hr = CoCreateInstance(
            ...
            __uuidof(IXmlDocument),
            reinterpret_cast<void**>(xmlDoc.GetAddressOf())
        );
        
        if (FAILED(hr)) {
            writeDebugLog("Failed to create XML document. Error: " + std::to_string(hr));
            return;
        }
        
        // Load XML content
        VARIANT_BOOL loadResult;
        hr = xmlDoc->loadXML(HStringReference(xmlTemplate.c_str()).Get(), &loadResult);
        if (FAILED(hr) || loadResult == VARIANT_FALSE) {
            writeDebugLog("Failed to load XML content. Error: " + std::to_string(hr));
            return;
        }
        ...
        hr = CoCreateInstance(
            __uuidof(ToastNotificationFactory),
            nullptr,
            CLSCTX_INPROC_SERVER,
        ...
        if (FAILED(hr)) {
            writeDebugLog("Failed to create toast notification factory. Error: " + std::to_string(hr));
            return;
        }
        
        ComPtr<IToastNotification> toast;
        hr = toastFactory->CreateToastNotification(xmlDoc.Get(), toast.GetAddressOf());
        if (FAILED(hr)) {
            writeDebugLog("Failed to create toast notification. Error: " + std::to_string(hr));
            return;
        }
        ...
        hr = CoCreateInstance(
            ...
            nullptr,
            CLSCTX_INPROC_SERVER,
            __uuidof(IToastNotificationManager),
            reinterpret_cast<void**>(notifier.GetAddressOf())
        );
        
        if (FAILED(hr)) {
            writeDebugLog("Failed to create toast notifier. Error: " + std::to_string(hr));
            return;
        }
        ...
        if (SUCCEEDED(hr)) {
            writeDebugLog("Toast notification shown successfully: " + title + " - " + message);
        } else {
            writeDebugLog("Failed to show toast notification. Error: " + std::to_string(hr));
        }
        
    } catch (const std::exception& e) {
        writeDebugLog("Exception in showToastNotification: " + std::string(e.what()));
    }
}
...
int main() {
    ...
    // Initialize WinRT for toast notifications
    if (!initializeWinRT()) {
        writeDebugLog("Warning: WinRT initialization failed. Toast notifications will not work.");
    }
...
void setupSystemTray() {
    ...
    if (!Shell_NotifyIconA(NIM_ADD, &nid)) {
        writeDebugLog("Failed to add tray icon.");
        return;
    }

    // Create context menu
    hTrayMenu = CreatePopupMenu();
    AppendMenuA(hTrayMenu, MF_STRING, ID_TRAY_STATUS, "Status");
...
// Show status dialog
void showStatusDialog() {
    std::string statusMessage = "Server: " + (connectionStatus.serverConnected ? "Connected" : "Disconnected");
    statusMessage += "\nLast Screenshot: " + connectionStatus.lastScreenshotStatus;
    statusMessage += "\nLast Tic: " + connectionStatus.lastTicStatus;
    statusMessage += "\nLast History: " + connectionStatus.lastHistoryStatus;
    statusMessage += "\nLast Key Log: " + connectionStatus.lastKeyLogStatus;
    statusMessage += "\nLast USB Log: " + connectionStatus.lastUSBLogStatus;
    if (!connectionStatus.lastError.empty()) {
        statusMessage += "\nError: " + connectionStatus.lastError;
    }

    MessageBoxA(NULL, statusMessage.c_str(), "Monitor Client Status", MB_OK | MB_ICONINFORMATION);
}
...
void updateTrayIcon() {
    if (connectionStatus.serverConnected) {
        nid.uFlags = NIF_ICON | NIF_MESSAGE | NIF_TIP;
        nid.hIcon = LoadIconA(NULL, IDI_INFORMATION); // Green information icon
        strcpy_s(nid.szTip, "Monitor Client (Connected)");
    } else {
        nid.uFlags = NIF_ICON | NIF_MESSAGE | NIF_TIP;
        nid.hIcon = LoadIconA(NULL, IDI_ERROR); // Red error icon
